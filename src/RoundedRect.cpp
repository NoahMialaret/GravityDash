...
{
  sf::Vector2f offset = newCentre - centre;
  centre = newCentre;
...
void RoundedRect::SetDim(sf::Vector2f newDim)
{
  ...
  left.setPosition({centre.x - newDim.x + 1.0f * Utility::gameScale, centre.y});

  right.setSize({Utility::gameScale, newDim.y - 2 * Utility::gameScale});
  right.setPosition({centre.x + newDim.x - 1.0f * Utility::gameScale, centre.y});
}
